...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data == 'error') return resolve({ creator: '@__a_n_i_r_u_d_h_t', status: false, message: 'cant be downloaded' })
                resolve({ creator: '@__a_n_i_r_u_d_h_', status: true, thumb: data.thumbnail, data: data.links })
            })
    })
}
